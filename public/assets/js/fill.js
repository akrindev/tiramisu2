...
const MAX_STEPS = 21; // max is fill +21
const SLOTS = 8; // max number of slots is 8
...
function show_details(pot) {
  ...
  buffer += '</table>';
  buffer += '<div class="mt-3 mr-5 text-right"> <img src="/img/potum.png" class="mr-4" width="50px" height="50px"> <span class="text-primary">toram-id.space</span></div>';
...
function saveAsImg() {
  ...
  html2canvas(srcDoc,{
    ...
    dl.href = img;
    dl.download = 'fill_stats '+ Math.floor(Math.random()*22) +'(toram-id.space).png'
    imgDoc.src = img;
  });
