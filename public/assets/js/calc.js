...
if(Number(lvnow) > Number(target_level)) {
  alert('level now greater than target level!!')
}
  ...
  $(".last_nc").text(lvnow < 50 ? 'min lv 50' : last_nc.toFixed(1)).css({color:'red'});
  $(".last_bnut").text(lvnow < 25 ? 'min lv 25' : last_bnut.toFixed(1)).css({color:'red'});
  $(".last_nwood").text(lvnow < 27 ? 'min lv 27' : last_nwood.toFixed(1)).css({color:'red'});

  var spina_nc = last_nc * nc;
  $(".spina_last_nc").text(lvnow < 50 ? 'min lv 50' : cm(Math.round(spina_nc))+'s').css({color:'blue'});

  var spina_bnut = last_bnut * bnut;
  $(".spina_last_bnut").text(lvnow < 25 ? 'min lv 25' : cm(Math.round(spina_bnut))+'s').css({color:'blue'});

  var spina_nwood = last_nwood * nwood;
  $(".spina_last_nwood").text(lvnow < 27 ? 'min lv 27' : cm(Math.round(spina_nwood))+"s").css({color:'blue'});
}
