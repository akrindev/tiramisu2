...
const MAX_STEPS = 20;
const BONUS_STEPS = 7; // 30 juni 2023
const SLOTS = 8;
...
class Slot {
    ...
    buildDisplay() {
        ...
        for (let data of OPTIONS) {
            if (this.stat.type === "a" && data.cat === "Awaken Elements")
                continue;
            ...
            cat_id++;
            buffer += `<option value="${cat_id}">${translate(
                data.name
            )}</option>`;
        }
    ...
    statToSteps(value = this.futureStat) {
        ...
        if (Math.abs(value) > max_normal_value) {
            ...
            future_steps =
                (max_normal_step +
                    (Math.abs(value) - max_normal_value) / overstep) *
                input_is_negative;
        } else {
    ...
    getMaxStat(isNega) {
        ...
        const custom_max =
            (this.stat_data.max || 0) * (this.stat_data.step || 1);
        ...
        if (this.stat_data.bonus) {
            ...
            if (isNega && this.stat_data.max_only)
                return custom_max || max_base_stat;
            return value;
        } else if (custom_max) {
            return custom_max;
        } else {
    ...
    getPotentialChange() {
        ...
        const bonuspot = Calc(bonus_diff)
            .multiply(this.stat_data.pot)
            .multiply(2);
    ...
    confirm() {
        if (this.stat_name) {
            ...
            document.getElementById(`slot${this.slot_num}`).style.color =
                "black";
        }
...
class Stat {
    ...
    calculatePenalty() {
        ...
        for (let slot of this.slots) {
            if (!slot.stat_name || (slot.new_stat && !slot.futureSteps))
                continue;
            if (!categories[slot.stat_data.cat])
                categories[slot.stat_data.cat] = 0;
    ...
    getCostReduction() {
        let percent =
            Math.floor(this.proficiency / 10) +
            Math.floor(this.proficiency / 50);
    ...
    confirm() {
        ...
        if (
            this.slots.every((slot) => slot.stat_name) ||
            this.future_pot <= 0
        ) {
    ...
    runStepInstruction(instruction) {
        ...
        this.pot = this.finished
            ? instruction.pot_before
            : instruction.pot_after;
...
class Formula {
    ...
    getDisplay() {
        ...
        const fill = this.condensed_formula
            .map(
                (step, index) =>
                    `<tr><td>${index + 1}</td> <td>${translate(
                        step.text
                    )} ${Object.entries(step.step_mats)
                        .filter(([k, v]) => v)
                        .map(
                            ([k, i]) =>
...
class MainApp {
    ...
    spawn(id) {
        ...
        const weap_arm = document.querySelector(
