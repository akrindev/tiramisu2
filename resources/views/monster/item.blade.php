    ...
    <div class="col-md-8">
        ...
        <div class="card">
          <div class="card-body p-3" style="font-size:14px;font-weight:400">
            <a href="/temp/drop/edit/{{ $item->id }}" rel="nofollow" target="_blank" class="float-right small text-muted"><span data-nosnippet> [<i class="">sarankan edit</i>]</span></a>

            <dl>
                <dt>
                    <b class="h5">
                    <img src="{{ $item->dropType->url }}"
                        class="avatar avatar-sm mr-2">
                            <a href="{{ app()->isLocale('en') ? '/en' : '' }}/item?name={{ $item->name }}&did={{ $item->dropType->id }}" class="text-primary">{{ $item->name }}</a>
                    </b>
                        ...
                        <ul class="nav nav-tabs justify-content-center" id="statusTab" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link {{ !is_null(optional($item->note)['monster']) || is_null(optional($item->note)['npc']) ? 'active' :  '' }}"
                                    ...
                                    role="tab" aria-controls="status" aria-selected="{{ !is_null(optional($item->note)['monster']) ? 'true' : 'false' }}">
                                    Status
                            ...
                            <li class="nav-item">
                                <a class="nav-link{{ is_null(optional($item->note)['monster']) && !is_null(optional($item->note)['npc']) ? ' active' :  '' }}"
                                    id="status-npc-tab" data-toggle="tab" href="#status-npc"
                                    role="tab" aria-controls="status-npc" aria-selected="{{ is_null(optional($item->note)['monster']) && !is_null(optional($item->note)['npc']) ? 'true' :  'false' }}">
                                    Status: NPC
                            ...
                            <li class="nav-item">
                                <a class="nav-link" id="mats-tab" data-toggle="tab" href="#mats" role="tab" aria-controls="mats" aria-selected="false">
                                    Craft: Player
            ...
            <div class="card-body p-3">
                <div>
                    ...
                    <div class="mb-5">
                        <dt class="mb-1">
                            <b class="h6">
                                <a class="text-primary" href="/monster/{{ $mons->id }}">{{ $mons->name }} (Lv {{ $mons->level }}) </a>
                                @switch($mons->type)
                                    @case(2)
                        ...
                        <details>
                            ...
                            @foreach ($mons->drops as $drop)
                                <a href="{{ request()->segment(1) == 'en' ? '/en' : '' }}/item/{{ $drop->id }}">
                                    <img src="{{ $drop->dropType->url }}" class="avatar avatar-sm"> {{ $drop->name }}
                                </a>
