...
@section('title', 'Toram Quest: '. $quest->name)
@section('description', 'Toram quest: '.$quest->name.', list npc,toram list quest, reward, npc picture and more')
@section('image', $quest->npc->picture ?? to_img())

@section('content')
<div class="my-5">
  <div class="container">
    <div class="page-header">
      <h3 class="page-title">Toram Quest: {{ $quest->name }}</h3>
    </div>
  ...
  @includeWhen(env('APP_ENV') == 'production', 'inc.ads_mobile')
          <div class="card-body p-3" style="font-size:13px;font-weight:400">
            <div style="display:block;clear:both" class="mb-5">
              <img src="{{ $quest->npc->picture }}" style="width:100px;height:100px;float:left" class="mr-3">
           <i class="fe fe-wind"></i> <a href="/npc/npc-{{ $quest->npc->id }}">{{ $quest->npc->name }}</a>
              @if (auth()->check() && auth()->user()->isAdmin()) <span class="text-danger" onclick="delNpc({{ $quest->npc->id }})">[Hapus]</span> <a href="/npc/edit/{{ $quest->npc->id }}">[edit]</a>
              @endif
              <br>
              <small class="text-muted">( {{ $quest->npc->map->name }} )</small>
           </div>
