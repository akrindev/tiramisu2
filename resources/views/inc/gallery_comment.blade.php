...
@auth
        ...
        @endif
          <div class="card-body p-3">
            ...
            <textarea name="body" class="form-control {{ $errors->has('body') ? 'is-invalid': ''}}" rows=5></textarea>
            @if($errors->has('body'))
            <div class="invalid-feedback">{{ $errors->first('body') }}
            </div>
            @endif

            <button class="btn btn-pill btn-outline-primary float-right my-3">Komentari</button>
          </div>
          {!! form_close() !!}
   		</div>
@else
